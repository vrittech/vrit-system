...
# Periodic task schedules
app.conf.beat_schedule = {
    # Publish Scheduled Blogs (from the `blogs` app)
    'publish-scheduled-blogs-every-10-sec': {
        'task': 'blogs.tasks.publish_scheduled_blogs',  # <-- match task name inside blogs/tasks.py
        'schedule': schedule(timedelta(seconds=10)),    # <-- run every 10 seconds
    },
}
