...
class projectserviceViewsets(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], name="draggableService", url_path="drag-service")
    def Draggable(self, request, *args, **kwargs):
        ...
        try:
            target_obj = ProjectService.objects.get(position=target_position)
            goal_obj = ProjectService.objects.get(position=goal_position)
        ...
        if target_position < goal_position:
            ...
            affected_objs = ProjectService.objects.filter(
